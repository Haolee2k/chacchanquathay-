...
{ 
    ...
    $tongDT=$dienthoaiDB->tongSoDienthoai1Loai($_GET['catdt']);
}else
{
...
if(isset($_GET['catdt'])){
$dienthoais=$db->exeQuery("select madt,tendt ,hinh,gia  from dienthoai left join loai on dienthoai.maloai=loai.maloai where dienthoai.maloai=? limit $bd,".DT_1_TRANG,array($_GET['catdt']));
}
if($dienthoais  == null){
...
<div class="phantrang" style="text-align:center">
    ...
    for($i=1;$i<=$sotrang;$i++)
        ...
        if(isset($_GET['catdt']))
        echo '<a href="loaidt.php?catdt='.$_GET['catdt'].'&p='.$i.'"> ',$i,' </a>';
        else
            echo '<a href="loaidt.php?p='.$i.'"> ',$i,' </a>';
    ?>
</div>
