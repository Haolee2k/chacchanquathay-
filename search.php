...
$sachDB=new Dienthoai();
if(isset($_GET['loai']))
{ 
    //$tam=$db->exeQuery("select count(*) from sach where maloai=?",array($_GET['loai']),PDO::FETCH_NUM);
    $tongSach=$sachDB->tongSoDienthoai1Loai($_GET['loai']);
}else
{
    //$tam=$db->exeQuery("select count(*) from sach",array(),PDO::FETCH_NUM);
    $tongSach=$sachDB->tongSoDienthoai();
}
//$tongSach=$tam[0][0];
$page=isset($_GET['p'])?$_GET['p']:1;
$bd=($page-1)*DT_1_TRANG;
 $dienthoais=$db->exeQuery("select madt,tendt,mota, luotxem,hinh,gia from dienthoai where tendt like '%$st%' limit 12");
if($dienthoais==0)
{
    echo "Không có";
}
